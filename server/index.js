...
app.post("/api/zamow", async (req, res) => {
  ...
  const user_id = req.body.user_id;
    ...
    const sql = "INSERT INTO zamowienia (opis, kategoria, user_id, czy_zrealizowane, czy_zaplacone, user_img) VALUES (?, ?, ?, ?, 0, 0, ?)";
    db.query(sql, [opis, kategoria, user_id, user_img], (err, results) => {  if (err) throw err;
      res.json({ success: 1 })      
    });
});
